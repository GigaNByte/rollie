...
function rollie_excerpt(){
	...
	//if ( $page_for_posts_id ) { ?>
		...
		<?php
		if (( $is_display_content_post_page ||  (class_exists('Woocommerce')&& is_woocommerce()))){
			...
			?>
					...
					if ( $rollie_alt_cat_title ) {
						echo '<div class="rollie_parent_title '.$rollie_below.'rollie_f_headings_h2 rollie_category_title_text_color ">' . $rollie_alt_cat_title . '</div>';
					}
		...
		<?php }
			...
			if (class_exists('Woocommerce')&& is_woocommerce() && apply_filters( 'woocommerce_show_page_title', true ) ){
			 echo '<div class="  rollie_f_headings '.$rollie_line.' rollie_title_text_color">';
			 woocommerce_page_title(); 
			 echo '</div>';
			}else{
