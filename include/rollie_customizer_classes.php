...
public  $customizer_section_title;
private $customizer_section;
private static $font_data_def  = array( //root level defaults if defaults arent declared in arguments of this constructor
	"U" => array("default" => false,"ignore" => true),
	"ls" => array( "default" => 0,"ignore" => false),
	"lh" => array( "default" => 1.2,"ignore" => false),
	"max" => array( "default" => 16,"ignore" => false),
	"min" => array("default" => 16,"ignore" => false),
	"align" => array( "default" => 1,"ignore" => false),
	"h4_min" => array("default" => 20,"ignore" => true),
	"h4_max" => array("default" => 24,"ignore" => true),
	"h2_max" => array("default" => 30,"ignore" => true),
	"h2_min" => array("default" => 27,	"ignore" => true),
	'alt_enable'=>array("default" => 1,"ignore" => false),
	"alt" => array("default" => '',"ignore" => false),
	"alt_weight" => array("default" => 40,"ignore" => false),
	"disable_bold_italic" => array("default" => true,"ignore" => false),
...
foreach(Rollie_Font::$font_data_def as $key => $value){

	if (isset($data[$key])){
		foreach(Rollie_Font::$font_data_def[$key] as $key_2 => $value_2){	
			if (isset($data[$key][$key_2])){
		 	$this->font_data[$key][$key_2] = $data[$key][$key_2]; 

		 	}else{
	 		$this->font_data[$key][$key_2] = Rollie_Font::$font_data_def[$key][$key_2];	
		 }
		} 	
	}else{
		 $this->font_data[$key] = Rollie_Font::$font_data_def[$key];
	}
}
...
public	function add_customizer_controls(){
	...
	if(($this->font_data['h4_min']['ignore'] == false && $this->font_data['h4_max']['ignore'] == false) || ($this->font_data['h2_min']['ignore'] == false && $this->font_data['h2_max']['ignore'] == false)){
		...
		if ($this->font_data['h4_min']['ignore'] == false )$heading_repeats[] = "h4";
		if ($this->font_data['h2_min']['ignore'] == false )$heading_repeats[] = "h2";
		...
		# code...
			...
			$wp_customize->add_control(
				new Rollie_Slider_Custom_Control(
					...
					$this->font_set_name.'_'.$value.'_min',
					array(
						'label'       => esc_html__( 'Minimum font size for '.$value.' headings (px)' ),
						'description' => esc_html__( 'Should be less than max font size otherwise sets default' ),
						'section'     => $this->customizer_section,
						'input_attrs' => array(
							'min'  => 1,
			...
			$wp_customize->add_control(
				new Rollie_Slider_Custom_Control(
					...
					$this->font_set_name.'_'.$value.'_max',
					array(
						'label'       => esc_html__( 'Maximum font size for '.$value.' headings (px)' ),
...
public function add_font ()
{
	...
	if ($this->font_data['U']['ignore'] == false){
	...
	if ($this->font_data['align']['ignore'] == false){
	...
	if ($this->font_data['h2_min']['ignore'] == false){
	...
	if ($this->font_data['h4_min']['ignore'] == false){
	...
	if (  !empty($font_str) && !empty($font_array))
	{
		...
		if ($font_array ['alt_enable']==1 && is_object($font_obj))
		{
...
function rollie_fonts_customizer_css(){
	...
	foreach ($rollie_font_data as  $rollie_font_obj) {
		$rollie_font_google_data[] = $rollie_font_obj->add_font();
...
{
	 array_values(array_filter($rollie_google_s));
	...
	if (!empty($rollie_google_s)&& $rollie_google_s != false ){	
		...
		foreach ($rollie_google_s as $rollie_google_name ){
			if (is_null($rollie_google_name))	continue;
